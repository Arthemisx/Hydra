...
Testa calculations.py, auth, sessões e reports.
Roda com SQLite em memória (sem o PostgreSQL).
"""
import os
import json

# Força SQLite em memória para testes
...
check("Fluid em sessão inexistente: status 404", resp.status_code == 404,
      f"got {resp.status_code}")

# Equipe vê sessões de todos
resp = client.get("/api/sessions", headers=headers_team)
check("Team vê todas sessões: status 200", resp.status_code == 200)

# Criar segunda sessão para testar listagem
resp = client.post("/api/sessions", json={
    "pre_mass_kg": 68.0,
    "sport": "Natacao",
    "expected_duration_min": 45,
}, headers=headers_ath)
check("Segunda sessão criada: 201", resp.status_code == 201)
